...
function renderRadios(){
 ...
 list.innerHTML=radios.map(r=>{
   ...
   return `<article class="radio-card"><div class="radio-card-top"><div><div class="radio-source">${esc(source.toUpperCase())} ${live}</div><h3>${esc(name)}</h3><p>${esc(detail)}</p></div><div class="radio-volume"><span>VOLUME</span><strong>${Math.max(0,Math.min(100,Number(r.volume)||0))}%</strong></div></div><div class="radio-meta"><span>Network ID <b>#${net}</b></span>${r.plate?`<span>Plate <b>${esc(r.plate)}</b></span>`:''}<span>Status <b>${r.playing===false?'PAUSED':'PLAYING'}</b></span>${r.queueLength!=null?`<span>Queue <b>${Number(r.queueLength)||0}</b></span>`:''}</div><div class="actions"><button class="primary" data-radio-teleport="${net}">Teleport To Vehicle</button><button class="soft-danger" data-radio-stop="${net}">Stop Radio</button></div></article>`
 }).join('');
}
