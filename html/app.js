...
function showAdminReportAlert(report){
 const host=document.querySelector('#adminReportAlerts');if(!host||!report)return;
 const el=document.createElement('div');el.className='admin-report-alert';
 const target=report.target?` · Target #${esc(String(report.target))}`:'';
 el.innerHTML=`<div class="ara-top"><div class="ara-title">New Player Report</div><div class="ara-id">#${esc(String(report.id||'?'))}</div></div><div class="ara-meta">From ${esc(report.reporter||'Player')} (#${esc(String(report.reporterId||'?'))})${target}</div><div class="ara-message">${esc(report.message||'')}</div><div class="ara-actions"><button type="button" class="ara-dismiss">Dismiss</button></div>`;
 host.appendChild(el);
...
function go(tab){
 ...
 const list=tab==='players'?document.getElementById('playersList'):null;
...
function render(){
 ...
 $('#playersList').innerHTML=players.length?players.map(p=>`<article class="player-card player-card-clean"><div class="player-main"><div class="avatar">${esc(initials(p.name))}</div><div class="player-info"><b>${esc(p.characterName||'Character not loaded')}</b><div class="identity-line"><span>FiveM / Rockstar:</span> ${esc(p.rockstarName||p.name||'Unknown')}</div><div class="player-meta-row"><span class="mini-badge">ID #${p.id}</span><span class="mini-badge">${esc(p.jobLabel||p.jobName||'Unemployed')}</span></div></div><button type="button" class="primary manage-player-btn" data-player-info data-id="${p.id}">Manage Player</button></div></article>`).join(''):'<div class="player-card muted">No players match your search.</div>';
 renderReports();
}
...
(() => {
  ...
  document.addEventListener('wheel', (event) => {
    ...
    if(playersPage?.classList.contains('active') && playersList){
      playersList.scrollTop += event.deltaY;
    }else{
...
async function renderPlayerInfo(p){
 ...
 document.querySelector('#piBody').innerHTML=`<div class="pi-overview"><div class="pi-section"><h3>Identity</h3><div class="pi-grid"><div class="pi-stat"><span>Character</span><b>${esc(p.characterName||'Unknown')}</b></div><div class="pi-stat"><span>FiveM / Rockstar</span><b>${esc(p.rockstarName||p.name)}</b></div><div class="pi-stat"><span>Server ID</span><b>#${p.id}</b></div><div class="pi-stat"><span>Citizen ID</span><b>${esc(p.citizenid||'N/A')}</b></div></div></div><div class="pi-section"><h3>QBCore</h3><div class="pi-grid"><div class="pi-stat"><span>Job</span><b>${esc(p.jobLabel||p.jobName||'Unemployed')}</b></div><div class="pi-stat"><span>Grade</span><b>${esc(String(p.jobGradeName||p.jobGrade||0))} (${p.jobGrade||0})</b></div><div class="pi-stat"><span>Gang</span><b>${esc(p.gang||'None')}</b></div><div class="pi-stat"><span>Cash</span><b>${fmtMoney(p.cash)}</b></div><div class="pi-stat"><span>Bank</span><b>${fmtMoney(p.bank)}</b></div></div></div></div><div class="pi-section"><h3>Live Status</h3>${liveHtml}</div><div class="pi-section action-section"><h3>Player Actions</h3><div class="action-group"><span class="action-group-title">Movement & Observation</span><div class="pi-actions"><button data-pi-act="goto">Go To</button><button data-pi-act="bring">Bring</button><button data-pi-act="spectate">Spectate</button><button data-pi-extra="waypoint">Set Waypoint</button></div></div><div class="action-group"><span class="action-group-title">Player State</span><div class="pi-actions"><button data-pi-act="freeze">Freeze</button><button data-pi-act="unfreeze">Unfreeze</button><button data-pi-act="revive">Revive</button><button data-pi-act="heal">Heal</button><button data-pi-act="ragdoll">Ragdoll</button></div></div><div class="action-group"><span class="action-group-title">Appearance & Effects</span><div class="pi-actions"><button data-pi-act="stripclothes">Remove Clothes</button><button data-pi-act="restoreclothes">Restore Clothes</button><button data-pi-act="dogs">Wild Dogs</button></div></div><div class="action-group"><span class="action-group-title">Character & Economy</span><div class="pi-actions"><button data-pi-extra="giveitem">Give Item</button><button data-pi-extra="setjob">Set Job</button><button data-pi-extra="money">Manage Money</button><button data-pi-extra="warnings">Warnings</button></div></div><div class="action-group"><span class="action-group-title">Vehicles</span><div class="pi-actions"><button data-pi-extra="vehicles">Owned Vehicles</button><button data-pi-extra="vehiclemanage">Vehicle Management</button><button data-pi-extra="transfer">Transfer Vehicle</button></div></div><div class="action-group danger-group"><span class="action-group-title">Moderation</span><div class="pi-actions"><button class="soft-danger" data-pi-extra="kick">Kick</button><button class="soft-danger" data-pi-act="kill">Kill</button><button class="soft-danger admin-role-only" data-pi-act="fire">Set Fire</button><button class="soft-danger admin-role-only" data-pi-act="explodevehicle">Explode Vehicle</button><button class="soft-danger admin-role-only" data-pi-act="ban">Ban</button></div></div></div>`;
...
function openVehicleManagement(p){
 managementTarget=p;
 $('#vehicleManageTitle').textContent=`Vehicle Management — ${p.characterName||p.name}`;
 $('#vehicleManageModal').classList.remove('hidden');
}
$('#vehicleManageClose').onclick=()=>$('#vehicleManageModal').classList.add('hidden');
$('#vehicleManageDone').onclick=()=>$('#vehicleManageModal').classList.add('hidden');
document.addEventListener('click',async e=>{
